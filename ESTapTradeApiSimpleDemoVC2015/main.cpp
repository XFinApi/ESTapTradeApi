...
2.拷贝XTA_W32\Api\ESTap_vX.X.X.X_XXXXXXXX目录（X.X.X.X_XXXXXXXX为版本号）下的所有dll文件到可执行文件目录，
...
void MarketTest()
{
	...
	int err = -1;
#ifdef _DEBUG
	market = XFinApi_CreateMarketApi("XTA_W32/Api/ESTap_v9.0.3.11/XFinApi.ESTapTradeApid.dll", &err);
#else
	market = XFinApi_CreateMarketApi("XTA_W32/Api/ESTap_v9.0.3.11/XFinApi.ESTapTradeApi.dll", &err);
#endif
...
void TradeTest()
{
	...
	int err = -1;
#ifdef _DEBUG
	trade = XFinApi_CreateTradeApi("XTA_W32/Api/ESTap_v9.0.3.11/XFinApi.ESTapTradeApid.dll", &err);
#else
	trade = XFinApi_CreateTradeApi("XTA_W32/Api/ESTap_v9.0.3.11/XFinApi.ESTapTradeApi.dll", &err);
#endif
	if (err || !trade)
	{
